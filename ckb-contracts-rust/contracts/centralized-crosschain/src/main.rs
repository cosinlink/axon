...
fn verify_transfer() -> Result<(), Error> {
    ...
    debug!("pubkey_hash = {:?}", pubkey_hash.to_vec());
    ...
    let rec_id = RecoveryId::parse(proof.as_slice()[64]).map_err(|_e| Error::InvalidSignature)?;
